...
class MyConvLSTMCell(nn.Module):
    ...
    def forward(self, x, state):
        if state is None:
            state = (Variable(torch.randn(x.size(0), x.size(1), x.size(2), x.size(3)).cuda()),      # initial state is random !!!
                     Variable(torch.randn(x.size(0), x.size(1), x.size(2), x.size(3)).cuda()))
        ht_1, ct_1 = state                                                                          # state is composed by two elements
        it = F.sigmoid(self.conv_i_xx(x) + self.conv_i_hh(ht_1))                                    # the output is a function of the input
        ft = F.sigmoid(self.conv_f_xx(x) + self.conv_f_hh(ht_1))                                    # the function can be found at the bottom of page 5 of first paper!!!
