...
class makeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if numFrame <= self.seqLen:
            startFrame = 1
            frame_indexes = np.floor(np.linspace(0, numFrame-1, self.seqLen)).astype(int)     # oversampling
        else:
            if self.phase == 'train':
                startFrame = random.randint(1,
                                            numFrame - self.seqLen)  # if the number of frames is more that stack size, start from a "middle" point
            else:
