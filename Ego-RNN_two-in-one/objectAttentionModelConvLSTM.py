...
class attentionModel(nn.Module):
    def __init__(self, num_classes=61, mem_size=512):
        ...
        self.dropout = nn.Dropout(0.7)  # specified from paper
    ...
    def forward(self, inputVariable, stage):
        ...
        for t in range(inputFrameVariable.size(0)):
            ...
            # lstm consists of evaluating the next state from the previous one and the features coming from the layer4 weighted on attention
            # the function is the lstm cell in which there is a convolution
            # see MyConvLSTMCell.py !!!
        ...
        if stage == 2:
            ...
            feats_ms = feats_ms.view(bz, inputFrameVariable.size(0), h * w)  # add class dimension
